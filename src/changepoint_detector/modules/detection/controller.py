...
class Controller:
    ...
    def process_sync(self, data: np.ndarray) -> dict[strategy.ChangepointDetectStrategy, int]:
        res:dict[strategy.ChangepointDetectStrategy, int] = {}
        for strat in self.strategies:
            res[strat] = self._postprocess(strat(data))
        return res

    def calculate_metric_aic(self, data: np.ndarray)-> dict[strategy.ChangepointDetectStrategy, float]:
        res = {}
